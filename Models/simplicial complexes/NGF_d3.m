function [a,r,kn,kt,SC] = NGF_d3(N,s,beta,figure_l)
...
% SC cell array of the simplices
...
if (figure_l==1)
   filename=sprintf('NGF_s%d_positions_d3.txt',s);
   fid=fopen(filename,'w');
   for i=1:N,
       fprintf(fid, ' %f %f %f \n', r(i,1), r(i,2),r(i,3));
   end
...
if (figure_l==1)
   filename=sprintf('NGF_s%d_ex_triangles_d3.txt',s);
   fid=fopen(filename,'w');
   for in=1:nt,
       if (a_occ(nt)==1)
       fprintf(fid, ' %d %d %d \n', tri(nt,1), tri(nt,2),tri(nt,3));
       end
   end
   fclose(fid);
end
