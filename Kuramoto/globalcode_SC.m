...
te = 1; %time counter
    ...
    for t = 0:dt:Tmax %time loop
        ...
        r1up=abs(sum(XpsiB))/N3;

        thetap = w - r1down*sigma*B1*imag(Xtheta); 
        phip = what - sigma*r0*r1up*B1t*imag(Xpsi) - sigma*r1down*B2*imag(XpsiB);
...
Rpsi2up(kcount) = Rpsi2up_ave/(te/5);
   ...
   Rpsidown_ave = 0; %to store time averages
    ...
    for t = 0:dt:Tmax
        ...
        if(te > 4*(Tmax/dt)/5) %time average in final fifth
            ...
            RpsiBdown_ave = RpsiBdown_ave + r1up;
